    ...
    class MarketEvaluator
    {
        ...
        public static bool EvaluateMarketBasedOnSpreadConfiguration(Market marketData, SpreadConfiguration spreadConfiguration)
        {
            ...
            if (spreadConfiguration.UseCoinGecko && marketData.AggregatorQuote != null)
            {
                ...
                {
                    Logger.Instance.LogError($"Market {marketData.Symbol} has enough spread ({marketData.SpreadPercentage}) and volume ({marketData.QuoteVolume}), but price ({marketData.BidRate}) is higher than on aggregator ({marketData.AggregatorQuote.CurrentPrice})");
                    return false;
                }
            }
